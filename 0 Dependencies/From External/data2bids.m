...
if need_channels_tsv
  ...
  % +++ insertion of this part by Schneider 2020
  cfg.channels = repmat(cfg.channels,size(channels_tsv,1),1);
  cfg.channels.name = channels_tsv.name;
  channels_tsv = merge_table(cfg.channels, channels_tsv, 'name');
  channels_tsv = [channels_tsv(:,4:7),channels_tsv(:,1:3),channels_tsv(:,8:end)];
  channels_tsv.status = cfg.xtra.channel_status;
  channels_tsv.units(ismember(channels_tsv.type,'trigger')) = {nan};
  channels_tsv.low_cutoff(ismember(channels_tsv.type,'trigger')) = {nan};
  channels_tsv.high_cutoff(ismember(channels_tsv.type,'trigger')) = {nan};
  channels_tsv.notch(ismember(channels_tsv.type,'trigger')) = {nan};
  channels_tsv.type(ismember(channels_tsv.type,'trigger')) = {'TRIG'};
